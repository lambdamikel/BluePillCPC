...
def convert_midi_file(fn):
    ...
    for (time, msg) in messages:
        ...
        #print(time1 - last_time)
        #delta = round((time1 - last_time) / 3.48)
        delta = round((time1 - last_time) / 3.6)
...
def main():
    ...
    canvas1.create_text( 200, 25, text = "Load MIDI File, Convert, & Exit", font=myfontsmaller, fill = "yellow" )

    root.wm_title("Ultimate CPC MIDI Card MIDI Converter")
    ...
    canvas1.create_text(200, 300, text="Playback Speed (Default 358)", font=myfontsmaller, fill="yellow")
